module "label" {
  source     = "git::https://github.com/blackbookusa/terraform-terraform-label.git?ref=tags/0.1.3"
...
data "aws_iam_policy_document" "assume_role" {
  ...
  statement {
    ...
    principals {
      type = keys(var.principals)[count.index]
      identifiers = var.principals[keys(var.principals)[count.index]]
    }
  }
}

module "aggregated_assume_policy" {
  source           = "git::https://github.com/blackbookusa/terraform-aws-iam-policy-document-aggregator.git?ref=tags/0.1.3"
